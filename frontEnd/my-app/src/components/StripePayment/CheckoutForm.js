...
const CheckoutForm = () => {
  //Get the payloads Basic     url/{parameter}
  const params = useParams();

   // /Basic?amount=20, search params could use to get the information after the ?
  const [searchParams] = useSearchParams();
  const plan = params.plan;

  // get the value pair with the amount key
  const amount = searchParams.get("amount");
  const mutation = useMutation({
    mutationFn: createStripePaymentIntentAPI, // use the api in the apis package
  });

  //Stripe configuration
  const stripe = useStripe(); // get the stripe object and related packages
  const elements = useElements(); // the object
    ...
    try {
      ...
      if (mutation?.isSuccess) {
        const { error } = await stripe.confirmPayment({
          elements,
          clientSecret: mutation?.data?.clientSecret, // get the secret code from the backend
          confirmParams: {
